  ...
  >
    ...
    >
      ...
      <view
        ...
        class="boxStyle2"
        :style="getLinezIndex + getDurationStyle + getBoxStyle2"
      />
      <view
        v-if="animationMode === 'line3'"
        class="boxStyle2"
        :style="getLinezIndex + getBoxStyle3"
      />
    </scroll-view>
  </view>
</template>
...
export default {
  props: {
    ...
    activeStyle: {
      ...
      type: String,
      default:
        "bottom:0;left:50%;transform: translate(-50%,-100%);height: 6rpx;border-radius:4rpx;"
    },
    defaultStyle: {
      //line1模式未选中项的样式
      type: String,
  ...
  computed: {
    ...
    getLw() {
      ...
      console.log("getLw:" + lw);
      return lw;
    },
    getBoxStyle2() {
      const lw = this.getLw;
      const x =
        Number(this.width) * (this.getCurrent + 1) -
        Number(this.width) / 2 -
        lw / 2;
